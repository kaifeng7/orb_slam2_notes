...
/**
 * @brief Copy Constructor a new Frame:: Frame object
 * 
 * @param frame 
 */
      ...
      mpReferenceKF(static_cast<KeyFrame *>(NULL))
{
    ...
    if (mbInitialComputations)
    {
        ...
        mfGridElementWidthInv = static_cast<float>(FRAME_GRID_COLS) / (mnMaxX - mnMinX);  //inverse of grid element width
        mfGridElementHeightInv = static_cast<float>(FRAME_GRID_ROWS) / (mnMaxY - mnMinY); //inverse of grid element hegiht
      ...
      mTimeStamp(timeStamp), mK(K.clone()), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    ...
    // Set no stereo information,because monocular has not depth information
...
void Frame::AssignFeaturesToGrid()
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        int nGridPosX, nGridPosY; //in which grid
...
{
    if (flag == 0)
        (*mpORBextractorLeft)(im, cv::Mat(), mvKeys, mDescriptors); //overloaded () operator
    else
...
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit)
{
    ...
    const float viewCos = PO.dot(Pn) / dist; //computer the cos

    if (viewCos < viewingCosLimit) //far away from the camera center
        return false;
...
/**
 * @brief Undistort keypoints given OpenCV distortion parameters.
 * 
 */
void Frame::UndistortKeyPoints()
{
    if (mDistCoef.at<float>(0) == 0.0) //distortion coefficient equal to zero
    {
        mvKeysUn = mvKeys;
        return;
    }
...
/**
 * @brief Computes image bounds for the undistorted image
 * 
 * @param imLeft 
 */
void Frame::ComputeImageBounds(const cv::Mat &imLeft)
{
    if (mDistCoef.at<float>(0) != 0.0)
    {
        ...
        mat.at<float>(0, 0) = 0.0;//left top
        mat.at<float>(0, 1) = 0.0;
        mat.at<float>(1, 0) = imLeft.cols;//right top
        mat.at<float>(1, 1) = 0.0;
        mat.at<float>(2, 0) = 0.0;//left bottom
        mat.at<float>(2, 1) = imLeft.rows;
        mat.at<float>(3, 0) = imLeft.cols;//right bottom
