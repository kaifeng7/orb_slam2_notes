...
void TemplatedVocabulary<TDescriptor, F>::create(const std::vector<std::vector<TDescriptor>> &training_features)
{
  ...
  int expected_nodes = (int)((pow((double)m_k, (double)m_L + 1) - 1) / (m_k - 1));//节点总数量

  m_nodes.reserve(expected_nodes); // avoid allocations when creating the tree

  vector<pDescriptor> features;
  getFeatures(training_features, features);//把Descriptor二维数组转为数组指针
...
void TemplatedVocabulary<TDescriptor, F>::getFeatures(const vector<vector<TDescriptor>> &training_features, vector<pDescriptor> &features) const
{
  ...
  for (vvit = training_features.begin(); vvit != training_features.end(); ++vvit)
  {
    ...
    {
      features.push_back(&(*vit));//先取值再取地址
    }
  }
}
...
void TemplatedVocabulary<TDescriptor, F>::HKmeansStep(NodeId parent_id, const vector<pDescriptor> &descriptors, int current_level)
{
  ...
  if ((int)descriptors.size() <= m_k)//如果描述子数量少于组数
  {
  ...
  else
  {
    ...
    while (goon)
    {
      ...
      //unsigned int d = 0;
      for (fit = descriptors.begin(); fit != descriptors.end(); ++fit) //, ++d)//遍历每个descriptors
      {
        double best_dist = F::distance(*(*fit), clusters[0]);
        unsigned int icluster = 0;

        for (unsigned int c = 1; c < clusters.size(); ++c)//判断与哪个clusters最近
        {
        ...
        groups[icluster].push_back(fit - descriptors.begin());//将descriptors的id号加入应属于的groups
        current_association[fit - descriptors.begin()] = icluster;//此descriptor属于哪个cluster
      }
  ...
  {
    NodeId id = m_nodes.size();//第几个数id就是几
  ...
  if (current_level < m_L)
  {
    ...
    const vector<NodeId> &children_ids = m_nodes[parent_id].children;//通过node[parent_id].children 找到children的id
    for (unsigned int i = 0; i < clusters.size(); ++i)
    {
      ...
      {
        child_features.push_back(descriptors[*vit]);//从groups中取出每组descriptor的id号，在通过id号找到descriptor存到child_feature
      }
    ...
    const vector<pDescriptor> &pfeatures, vector<TDescriptor> &clusters) const
{
  ...
  int ifeature = DUtils::Random::RandomInt(0, pfeatures.size() - 1); //从n个样本中随机选取一个点作为第一个中心点
  ...
  {
    *dit = F::distance(*(*fit), clusters.back());//剩余每个点和第一个点的距离
  }

  while ((int)clusters.size() < m_k)
  {
    // 2.对于数据集中的每一个点x，计算它与最近聚类中心(指已选择的聚类中心)的距离Dx并保存在一个数组里，把这些距离加起来
    dit = min_dists.begin();
    for (fit = pfeatures.begin(); fit != pfeatures.end(); ++fit, ++dit)
    {
      if (*dit > 0)
    ...
    if (dist_sum > 0)
    {
      ...
      do
      {
        cut_d = DUtils::Random::RandomValue<double>(0, dist_sum);//取一个随机值[0,sum]
      } while (cut_d == 0.0);//不取0
      ...
      clusters.push_back(*pfeatures[ifeature]);//将第j个点设为种子点
